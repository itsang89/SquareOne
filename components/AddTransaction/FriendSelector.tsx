...
export const FriendSelector: React.FC<FriendSelectorProps> = ({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        {selectedFriend ? (
          <div className="flex items-center gap-3">
            ...
            <span className="text-lg font-bold">{selectedFriend.name}</span>
          </div>
        ) : (
          <div className="flex items-center gap-3">
      ...
      {isOpen && (
        <div className="bg-white border-2 border-black shadow-neo-lg mt-2 rounded-lg max-h-64 overflow-y-auto z-30">
          {friends.map(friend => (
            <button
              key={friend.id}
