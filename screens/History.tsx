...
import { Search, Pencil, Calendar, X } from 'lucide-react';
...
export const History: React.FC = () => {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return filtered;
  }, [transactions, searchQuery, activeFilter, friends, effectiveRange]);
  ...
  const hasNonDefaultFilter =
    activeFilter !== 'All' || datePreset !== 'all' || customRange.start || customRange.end || searchQuery.trim();

  // Build the unified pill list
  const pills = useMemo<Pill[]>(() => {
    ...
    TRANSACTION_TAGS.forEach(tag => {
      result.push({
        id: tag.label,
        label: tag.label,
        icon: TAG_ICON_MAP[tag.label] ?? <Tag size={12} />,
    ...
    const datePills: Pill[] = [
      {
        ...
        isActive: datePreset === '7days' && !customRange.start,
      },
      {
        id: '30days',
        label: '30d',
        ...
        isActive: datePreset === '30days' && !customRange.start,
      },
      {
        id: '3months',
        label: '3m',
        ...
        isActive: datePreset === '3months' && !customRange.start,
      },
    ];

    if (customRange.start || customRange.end) {
      const label = `${customRange.start ? new Date(customRange.start).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : ''} – ${customRange.end ? new Date(customRange.end).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : ''}`;
      datePills.push({
        id: 'custom',
        label: label.trim(),
        icon: <Calendar size={12} />,
    ...
    return [...result, ...datePills];
  }, [activeFilter, datePreset, customRange, customTypes]);
  ...
  return (
    <div className="min-h-screen bg-neo-bg dark:bg-zinc-950 flex flex-col pb-24 relative overflow-hidden transition-colors duration-300">
      ...
      {/* Single unified pill row */}
      <div className="px-4 pt-3 pb-2 bg-neo-bg dark:bg-zinc-950 shrink-0 z-10">
        <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-1 items-center">
          {pills.map(pill => (
            <button
              key={pill.id}
              onClick={pill.onClick}
              className={`shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border-2 font-bold text-xs transition-all ${
                pill.isActive
                  ? 'bg-black text-white dark:bg-zinc-100 dark:text-black border-black shadow-neo-sm'
                  : 'bg-white dark:bg-zinc-900 dark:text-zinc-100 border-black/30 dark:border-white/20 hover:border-black dark:hover:border-white hover:bg-gray-50 dark:hover:bg-zinc-800'
              }`}
            >
              {pill.icon}
          ...
          {hasNonDefaultFilter && (
            <button
              ...
              className="shrink-0 flex items-center gap-1 px-2.5 py-1.5 rounded-md border-2 border-neo-red text-neo-red font-bold text-xs hover:bg-neo-red/10 transition-all ml-1"
            >
              <X size={11} /> Clear
            </button>
          )}
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-4 pt-2">
        {/* Inline search — shown only when active */}
        <div className="relative mb-3">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <Search className="text-black dark:text-zinc-100" size={16} />
          </div>
          <input
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search friend, note, amount..."
            className="w-full pl-9 pr-8 py-2 rounded-md border-2 border-black bg-white dark:bg-zinc-900 dark:text-zinc-100 font-bold text-sm placeholder:text-gray-400 shadow-neo-sm focus:outline-none focus:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-shadow"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery('')}
              className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-black dark:hover:text-zinc-100"
            >
              <X size={14} />
            </button>
          )}
              ...
              >
                {txs.map(tx => {
                  ...
                  return (
                    <motion.div
                      key={tx.id}
                      className={`group relative flex items-center gap-3 p-3 mb-3 rounded-md border-2 border-black shadow-neo ${tx.payerId === 'me' ? 'bg-neo-green/20 dark:bg-neo-green/10' : 'bg-neo-red/20 dark:bg-neo-red/10'} ${isGrayed ? 'opacity-50 grayscale' : tx.isSettlement ? 'opacity-60' : ''}`}
                    ...
                    >
                      <div className="relative shrink-0">
                        ...
                        {!tx.isSettlement && (
                          <div className={`absolute -bottom-1 -right-1 border-2 border-white dark:border-zinc-950 rounded-full w-4 h-4 ${tx.payerId === 'me' ? 'bg-neo-green' : 'bg-neo-red'}`} />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                          ...
                          <span className={`font-bold whitespace-nowrap px-1 rounded ${isGrayed ? 'text-gray-500 dark:text-zinc-500 bg-white/50 dark:bg-zinc-800/50' : tx.payerId === 'me' ? 'text-green-700 dark:text-neo-green bg-white/50 dark:bg-zinc-800/50' : 'text-red-700 dark:text-neo-red bg-white/50 dark:bg-zinc-800/50'}`}>
